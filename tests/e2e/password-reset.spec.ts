...
test.describe('E2E Password Reset Flow', () => {
  ...
  test('Forgot password form validation', async ({ page }) => {
    ...
    // Submit invalid email — native HTML5 validation blocks form submission
    await page.fill('input[name="email"]', 'not-an-email');
    await page.click('button[type="submit"]');

    // Form should NOT advance to the success state
    await expect(page.getByRole('heading', { name: /Forgot Password/i })).toBeVisible({ timeout: 3000 });
    await expect(page.getByText('Check Your Email')).not.toBeVisible();
  });

  test('Reset password form validation', async ({ page }) => {
    // Navigate to reset page with a fake token — client-side validation fires
    // before the form is submitted to the server, so no real token is needed
    await page.goto('http://localhost:3000/auth/reset-password?token=fake.validation.token');

    // Test weak password (< 8 chars)
    await page.fill('input[name="password"]', 'weak');
    await page.fill('input[name="confirmPassword"]', 'weak');
    await page.click('button[type="submit"]');
    await expect(page.locator('div.bg-red-100'))
    ...
    await page.click('button[type="submit"]');
    await expect(page.locator('div.bg-red-100'))
      .toContainText(/passwords.*match|do not match/i, { timeout: 3000 });
  });

  test('Expired token shows error with link to request new reset', async ({ page }) => {
    // Token is present but invalid — page shows the form, error appears after submit
    ...
    // Server returns error for invalid/expired token
    await expect(page.locator('div.bg-red-100'))
      .toContainText(/expired|invalid/i, { timeout: 5000 });
